# -*- coding: utf-8 -*-
"""
Created on Wed Nov 27 14:49:23 2019

@author: Elina

copy pasted from https://github.com/matt-j-harvey/Calcium-Imaging-Pipeline/blob/master/pipeline%203%20delta%20f
"""
...
def load_cell_trace_tiff_file(plane, file_number):
    ...
    file_name = str(
        folder_location + r"\\plane" + str(plane) + r"\\reg_tif\\file" + str(file_number).zfill(3) + "_chan0.tif")
...
def threshold_cells():
    ...
    for plane in range(number_of_planes):
        for cell in range(len(neuron_decisions[plane])):
            if cell_probability_decisions[plane][cell] and within_region_decisions[plane][cell] and \
                    cell_noise_decisions[plane][cell]:
...
def plot_flouresence_traces(plane, cell_from, cell_to):
    ...
    for cell in range(cell_from, cell_to):
        if neuron_decisions[plane][cell] == True:
            ...
            spike_trace = spike_matricies[plane][cell]

            # Plot Traces On Canvas
            plot_trace_graph([spike_trace],
                             cell_trace_figures[plane][cell], cell_trace_canvases[plane][cell], "Neuron: " + str(cell),
                             plane, cell)
...
def lowpass_filter(trace, frequency_cutoff):
    ...
    fast_fourier_transform[frequency_cutoff + 2:(len(
        fast_fourier_transform) - frequency_cutoff)] = 0  # Remove All Components Whos Frequency Is Greater Than The Cuttoff Frequency
...
def bcl_function_parameters(trace, wdt, lamb, varB, varC, Cmean, frequency, fourier_cutoff):
    ...
    for t in range(1, N):
        ...
        logp0 = log(1 - wdt) - 0.5 * log(2 * pi) - 0.5 * log(varX + varB * dt) - (y[t] - cnew - B[t - 1]) ** 2 / (
                    2 * varX + 2 * varB * dt)

        cspike = Cmean + cnew + (y[t] - cnew - B[t - 1]) / (1 + varB * dt / varC + varX / varC)
        Bspike = B[t - 1] + varB * dt / varC * (cspike - cnew - Cmean)

        logp1 = log(wdt) - 0.5 * log(2 * pi) - 0.5 * log(varX + varB * dt + varC) - (
                    y[t] - cnew - B[t - 1] - Cmean) ** 2 / (2 * varX + 2 * varB * dt + 2 * varC)
...
def extract_delta_f(trace):
    f_min = min(trace)
    trace = np.add(trace,abs(f_min))
    f_zero = np.percentile(trace, 5)
    delta_f = np.subtract(trace, f_zero)
    delta_f_over_f = np.divide(delta_f, f_zero)
#    print(f_zero)
#    filtered_delta_f_over_f = lowpass_filter(delta_f_over_f,500)
    ...
    for plane in range(number_of_planes):
        ...
        for neuron in range(len(neuron_decisions[plane])):
            if neuron_decisions[plane][neuron] == True:
                ...
                delta_f_over_f = extract_delta_f(flourescence_traces[plane][neuron])
                spike_matricies[plane][neuron] = delta_f_over_f
